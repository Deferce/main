...
class Registration extends Connect
{
    ...
    protected function checkForDuplicateUsername(string $username)
    {
    ...
    protected function checkForDuplicateEmail(string $email)
    {
    ...
    protected function fieldValidation(string $username, string $lastname, string $email, string  $password,  string $passwordre, string $submit)
    {
        ...
        if (isset($submit)) {
            ...
            //dublicate check for username
            if ($this->checkForDuplicateUsername($username) == false) {
                header('Location: ../registration.php?username=dublicateUser&user=' . urlencode(base64_encode($username)));

                exit();
            }
            // dublicate check for email


            if ($this->checkForDuplicateEmail($email) == false) {
                header('Location: ../registration.php?email=dublicateEmail&em=' . urlencode(base64_encode($email)));
